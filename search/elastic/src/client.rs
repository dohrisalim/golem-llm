...
use crate::bindings::golem::search_elastic::types::{
...
impl ElasticClient {
    ...
    pub fn create_index(&self, _name: IndexName, _schema: Option<Schema>) -> Result<(), SearchError> {
        Err(SearchError::Internal("Elasticsearch operations not yet implemented in WASM".to_string()))
    }
    ...
    pub fn stream_search(&self, _index: IndexName, _query: SearchQuery) -> Result<std::collections::VecDeque<SearchHit>, SearchError> {
        // For now, return unsupported since streaming is complex in WASM
        Err(SearchError::Internal("Streaming not supported in WASM".to_string()))
    }
