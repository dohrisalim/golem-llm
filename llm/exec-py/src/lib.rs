...
pub fn decode_content(file: &File) -> Result<Vec<u8>, ExecError> {
...
fn execute_python(
    ...
    // Add arguments to the Python process
    for arg in &args {
        cmd.arg(arg);
    }
        ...
        if let Some(time_ms) = constraints.time_ms {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                match child.wait_timeout(Duration::from_millis(time_ms))? {
                    ...
                    None => {
                        // Timeout exceeded, kill the process and return timeout error
...
impl ExecutionSession {
    pub fn new(language: Language) -> Result<Self, Error> {
        if language.kind != LanguageKind::Python {
    ...
    pub fn upload_file(&mut self, file: File) -> Result<(), Error> {
    ...
    fn execute_code(&self, entrypoint: &str, args: Vec<String>, env: Vec<(String, String)>, stdin: Option<String>, constraints: Option<Limits>) -> Result<ExecResult, Error> {
        ...
        let code = String::from_utf8(code_content.clone())
            .map_err(|e| Error::Internal(format!("Invalid UTF-8 in Python code: {}", e)))?;

        execute_python(&code, args, env, stdin, constraints)
            .map_err(Into::into)
    }
}
...
impl ExecutorGuest for Executor {
    fn run(
        ...
        let main_file = files.iter()
            .find(|f| f.name == "main.py" || f.name == "index.py")
            .or_else(|| files.first())
            .ok_or_else(|| Error::Internal("No Python files provided".to_string()))?;
